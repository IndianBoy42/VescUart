...
public:
  ...
  /** Call this every so often to advance the reception FSM*/
  int receiveUartMessageFsm();
   ...
   * @return     True if successfull otherwise false
   */
  bool getValuesMasked(uint32_t mask);
   ...
   * @return     True if successfull otherwise false
   */
  bool getValuesMasked(uint32_t mask, uint8_t canId);
  ...
  /**
   * @brief      Set the rpm of the motor
   * @param      rpm  - The desired Position
   * @param      canId  - The CAN ID of the VESC
   */
...
private:
   ...
   */
  int receiveUartMessageBlocking(uint8_t *payloadReceived);
  void receiveUartMessageReady(uint16_t expectedLen);
